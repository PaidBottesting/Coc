...
class CoCAPI:
    def __init__(self, api_key):
        self.base_url69 = "https://api.clashofclans.com/v1"
    ...
    async def fetch_player(self, tag):
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.base_url69}/players/%23{tag.strip('#')}", headers=self.headers) as resp:
...
class BrawlAPI:
    def __init__(self, api_key):
        self.base_url69 = "https://api.brawlstars.com/v1"
        self.headers = {"Authorization": f"Bearer {api_key}"}
        self.max_brawlers = 91  # Update if new brawlers are released
...
class ClashRoyaleAPI:
    def __init__(self, api_key):
        self.base_url69 = "https://api.clashroyale.com/v1"
        self.headers = {"Authorization": f"Bearer {api_key}"}
...
async def check_brawl_stars(message: Message):
    c.execute('SELECT expire_at FROM users WHERE user_id=?', (message.from_user.id,))
    row = c.fetchone()
    if not row or datetime.datetime.fromisoformat(row[0]) < datetime.datetime.utcnow():
        await message.answer("❌ <b>No Access!</b> Use /redeem key")
    ...
    if len(args) < 2:
        await message.answer("⚠️ Usage: /check_bs player_tag")
        return
    tag = args[1]
    ...
    response += f"🏆 <b>Trophies:</b> {data.get('trophies', 'N/A')}\n"
    response += f"🏅 <b>Highest Trophies:</b> {data.get('highestTrophies', 'N/A')}\n"
    response += f"🌐 <b>Region:</b> {region}\n"
    response += f"🇳 <b>Possible Country:</b> {possible_country}\n"
    if 'club' in data and data['club']:
        response += f"👥 <b>Club:</b> {html.escape(data['club'].get('name', 'None'))} ({data['club'].get('tag', 'N/A')})\n"
    else:
        response += f"👥 <b>Club:</b> None\n"
    response += f"⚔️ <b>3v3 Wins:</b> {data.get('3vs3Victories', '0')}\n"
    response += f"🥊 <b>Solo Victories:</b> {data.get('soloVictories', '0')}\n"
    response += f"🤝 <b>Duo Victories:</b> {data.get('duoVictories', '0')}\n"

    await message.answer(response)

@router.message(Command("check_cr"))
async def check_clash_royale(message: Message):
    c.execute('SELECT expire_at FROM users WHERE user_id=?', (message.from_user.id,))
    row = c.fetchone()
    if not row or datetime.datetime.fromisoformat(row[0]) < datetime.datetime.utcnow():
        await message.answer("❌ <b>No Access!</b> Use /redeem key")
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.answer("⚠️ Usage: /check_cr player_tag")
        return
    tag = args[1]
    ...
    response += f"📊 <b>Level:</b> {data.get('expLevel', 'N/A')}\n"
    response += f"🏆 <b>Trophies:</b> {data.get('trophies', 'N/A')}\n"
    response += f"🏅 <b>Highest Trophies:</b> {data.get('bestTrophies', 'N/A')}\n"
    response += f"🌐 <b>Region:</b> {region}\n"
    response += f"🇳 <b>Possible Country:</b> {possible_country}\n"
    if 'clan' in data:
        response += f"👥 <b>Clan:</b> {html.escape(data['clan'].get('name', 'None'))} ({data['clan'].get('tag', 'N/A')})\n"
    else:
        response += f"👥 <b>Clan:</b> None\n"
    response += f"⚔️ <b>Wins:</b> {data.get('wins', '0')}\n"
    response += f"🛡️ <b>Losses:</b> {data.get('losses', '0')}\n"
    response += f"🎮 <b>Battles Played:</b> {data.get('battleCount', '0')}\n"

    await message.answer(response)

@router.message(Command("brawler"))
async def brawler_summary(message: Message):
    c.execute('SELECT expire_at FROM users WHERE user_id=?', (message.from_user.id,))
    row = c.fetchone()
    if not row or datetime.datetime.fromisoformat(row[0]) < datetime.datetime.utcnow():
        await message.answer("❌ <b>No Access!</b> Use /redeem key")
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.answer("⚠️ Usage: /brawler player_tag")
        return
    tag = args[1]
    ...
    star_power_count = sum(1 for brawler in brawlers if brawler.get('starPowers'))
    gadget_count = sum(1 for brawler in brawlers if brawler.get('gadgets'))

    response = f"⭐ <b>{safe_name}'s Brawler Summary</b> ⭐\n"
    response += f"🏷️ <b>Tag:</b> {data.get('tag', tag)}\n"
    response += f"🥊 <b>Brawlers:</b> {brawler_count}/{brawl_api.max_brawlers}\n"
    response += f"✨ <b>Star Powers:</b> {star_power_count}\n"
    response += f"🔧 <b>Gadgets:</b> {gadget_count}\n"
...
async def card_details(message: Message):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.answer("⚠️ Usage: /cards player_tag")
        return
    tag = args[1]
    ...
    max_level_cards = sum(1 for card in cards if card.get('level') == card.get('maxLevel'))

    response = f"🃏 <b>{safe_name}'s Clash Royale Cards</b> 🃏\n"
    response += f"🏷️ <b>Tag:</b> {data.get('tag', tag)}\n"
    response += f"📜 <b>Total Cards:</b> {card_count}\n"
    response += f"🔝 <b>Max Level Cards:</b> {max_level_cards}\n"
    response += "\n<b>Top Cards:</b>\n"
    # List top 5 cards by level to avoid flooding
    sorted_cards = sorted(cards, key=lambda x: x.get('level', 0), reverse=True)[:5]
    for card in sorted_cards:
        name = html.escape(card.get('name', 'Unknown'))
        level = card.get('level', 'N/A')
        max_level = card.get('maxLevel', 'N/A')
        response += f"- {name}: Level {level}/{max_level}\n"
...
/check player_tag [obstacles] - Get CoC player stats 📊
/check_bs player_tag - Get Brawl Stars player stats 🎮
/check_cr player_tag - Get Clash Royale player stats 👑
/brawler player_tag - Get Brawl Stars brawler summary ⭐
/cards player_tag - Get Clash Royale card details 🃏
...
/addinfo tag creation_date last_seen devices transactions obstacles skins - (Admin) Add player data ✍️
