...
/skin <code>player_tag</code> - Get skin count for CoC or Brawl Stars 🎨
...
async def brawler_details(message: Message):
    ...
    total_hypercharges = sum(1 for b in brawlers if b.get('power', 0) >= 11 and 'hypercharge' in b and b['hypercharge'])
    ...
    response += f"⚡ <b>Hypercharge Brawlers:</b> {total_hypercharges}\n"
    await message.answer(response)

@router.message(Command("skin"))
async def skin_count(message: Message):
    c.execute('SELECT expire_at FROM users WHERE user_id=?', (message.from_user.id,))
    row = c.fetchone()
    if not row or datetime.datetime.fromisoformat(row[0]) < datetime.datetime.utcnow():
        await message.answer("❌ <b>No Access!</b> Use /redeem <code>key</code>")
    ...
    args = message.text.split()[1:]
    if not args:
        await message.answer("⚠️ Usage: /skin <code>player_tag</code>")
        return
    tag = args[0]
    # Try Brawl Stars first
    brawl_data, brawl_error = await brawl_api.fetch_player(tag)
    if brawl_data:
        safe_name = html.escape(brawl_data.get('name', 'Unknown'))
        brawlers = brawl_data.get('brawlers', [])
        skins = sum(len(b.get('skins', [])) for b in brawlers if 'skins' in b)
        response = f"🎨 <b>{safe_name}'s Brawl Stars Skins</b> 🎨\n"
        response += f"🏷️ <b>Tag:</b> {brawl_data.get('tag', tag)}\n"
        response += f"👾 <b>Total Skins:</b> {skins}\n"
        await message.answer(response)
        return
    # Then try Clash of Clans
    coc_data, coc_error = await coc_api.fetch_player(tag)
    if coc_data:
        safe_name = html.escape(coc_data.get('name', 'Unknown'))
        hero_skins = sum(len(h.get('skins', [])) for h in coc_data.get('heroes', []))
        response = f"🎨 <b>{safe_name}'s CoC Skins</b> 🎨\n"
        response += f"🏷️ <b>Tag:</b> {coc_data.get('tag', tag)}\n"
        response += f"🗡️ <b>Hero Skins:</b> {hero_skins}\n"
        await message.answer(response)
        return
    await message.answer(brawl_error or coc_error or "❌ <b>Player Not Found!</b>\nCheck the tag and try again. 🏷️")
