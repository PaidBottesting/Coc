...
def guess_country_by_name(name):
    ...
    # Check for Chinese characters first (Unicode range for Hanzi: U+4E00 to U+9FFF)
    if re.search(r'[\u4e00-\u9fff]', name):
        return "China 🇨🇳 (based on name)"
    
    # Common names by country (case-insensitive, partial matches allowed)
    ...
    # Check each country’s name list
...
async def card_details(message: Message):
    ...
    # Show first 5 cards to keep it concise
    for card in cards[:5]:
